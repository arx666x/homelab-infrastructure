...
| `deploy-dsm.sh` meldet beim `method=import`-Call `{"error":{"code":119},"success":false}`, obwohl Login (SID-Erwerb) zuvor erfolgreich war | Synology-API-Fehler 119 = "SID not found" - die Session wurde zwischen `auth.cgi login` und dem eigentlichen Import ungültig, vermutlich durch einen zeitgleichen DSM-seitigen Vorgang (z.B. automatischer Neustart/Update rund um die Cron-Zeit 03:20 Uhr). Live bestätigt am 2026-09-13: einmaliger, transienter Aussetzer - TrueNAS und Pi-hole liefen im selben Lauf fehlerfrei durch, ein manueller Retest direkt danach zeigte für alle drei Ziele bereits "Fingerprint stimmt überein" (das Zertifikat war serverseitig also tatsächlich schon korrekt importiert, nur die Erfolgsantwort kam wegen der ungültig gewordenen Session nicht sauber zurück) | Kein Code-Fix nötig - Cronjob läuft am nächsten Tag ohnehin erneut, Fingerprint-Vergleich verhindert unnötige Doppel-Imports. Bei Wiederholung an mehreren Tagen in Folge: DSM-eigenes Auto-Update-/Neustart-Zeitfenster (Systemsteuerung → Update & Wiederherstellung) gegen die Cron-Zeit `20 3 * * *` prüfen und ggf. entzerren |
