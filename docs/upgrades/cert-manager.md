...
- **`deploy-direct.sh` und `gitops/apps/cert-manager.yaml` können auseinanderlaufen**, wenn nach einem Upgrade vergessen wird, das Bootstrap-Script zu aktualisieren — regelmäßig gegenprüfen. War beim v1.21.0-Upgrade bereits der Fall (Script stand noch auf v1.19.4, obwohl produktiv v1.20.3 lief).
- **Ab v1.21.0: ACME-Solver-Secret-Änderungen triggern keine Reconciliation** ([Issue #9036](https://github.com/cert-manager/cert-manager/issues/9036), offen). Wird `cloudflare-api-token` gelöscht/neu angelegt (z.B. bei Rotation) und der Issuer dadurch `Ready:False/InvalidSolver`, reicht das bloße Wiederanlegen des Secrets nicht — Issuer/ClusterIssuer-Spec manuell anfassen (z.B. no-op Annotation), Controller neu starten, oder bis zu 10h auf den Informer-Resync warten.
- **Post-Upgrade-Log-Grep liefert beim Controller-Neustart immer False Positives:** kurzer Error-Burst `"ACME client for issuer not initialised/available"` (~2s, bis `verified existing registration with ACME server` geloggt wird) sowie das kosmetische Log-Spam-Issue `"OnAdd missing Object"` ([#8994](https://github.com/cert-manager/cert-manager/issues/8994), seit v1.21.0, betrifft Issuer/ClusterIssuer-Controller nicht) — beides ignorierbar, kein echter Fehler.
