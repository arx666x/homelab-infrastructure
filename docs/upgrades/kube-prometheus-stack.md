...
- **Aktuelle Version:** 91.6.0 (Helm Chart; Prometheus-Operator v0.94.1)
...
| 2026-08-10 | 88.1.3 → 88.2.0 | Minor | Automatisch | Abgeschlossen | Automatisches Minor-Update durch homelab-version-checker, kein Operator-/CRD-Change (Operator bleibt v0.93.0); kube-state-metrics 8.1.3→8.2.0, Grafana 12.10.1→12.10.4, kein Breaking Change | Commit `f151da4` ("chore: auto-upgrade kube-prometheus-stack 88.1.3 → 88.2.0"), bereits vor dieser Session vom upgrade-agent committet und von ArgoCD gesynct; im Rahmen des Cluster-Upgrades am selben Tag nur nachverifiziert (Grafana `/api/health` 200, alle Pods Running) |
| 2026-08-21 | 88.3.0 → 88.5.3 | Minor | Manuell | Abgeschlossen | Operator nur Patch-Bump v0.93.0→v0.93.1 (laut Release-Notes zwei reine Bugfixes, keine CRD-Schema-Änderung), kein Dependency-Major-Bump (kube-state-metrics 8.2.0→8.4.0, Grafana 12.10.4→12.11.1, beides Minor) — daher ohne CRD-Pre-Apply direkt via GitOps gefahren | Teil der Sammel-Update-Runde 2026-08-21; Chart.yaml-Diff der Ziel-/Ausgangsversion vorab per GitHub geprüft, kein Cluster-Zugriff aus dieser Session verfügbar — Post-Upgrade-Verifikation (Pods/ArgoCD-Sync/Grafana-Health) muss lokal nachgeholt werden |
| 2026-08-24 | 88.5.3 → 88.5.4 | Minor (Patch) | Automatisch | Abgeschlossen | Commit `29c4ba5`, bislang nicht in diesem Changelog erfasst, jetzt nachgetragen | — |
| 2026-08-30 | 88.5.4 → 88.6.1 | Minor | Manuell | Abgeschlossen | Kein Operator-Bump (bleibt v0.93.1), keine CRD-Änderungen laut Commit-Notiz `af0b86f` — nur additive, optionale Gateway-API-Route-Unterstützung in Chart-Templates/values.yaml (nicht genutzt, Traefik-Ingress statt Gateway API) | Bislang nicht in diesem Changelog erfasst, jetzt nachgetragen |
| 2026-09-13 | 88.6.1 → 90.2.0 | Major (nominell, Chart-Version) | Manuell | Abgeschlossen | Nominell ein großer Sprung (88→90, 89.x komplett übersprungen), aber Operator bleibt v0.93.1 **unverändert** auf beiden Seiten bestätigt (kein CRD-Update nötig) — der Chart-Versionssprung stammt fast ausschließlich aus Dependency-Bumps: kube-state-metrics → 8.4.2, prometheus-node-exporter → 4.57.0, Grafana → 13.2.4 (bei uns bereits seit 88.x auf v13.2.0, also nur ein Patch-Sprung, nicht der eigentliche v12→v13-Breaking-Change-Schritt aus dem 89.x-Release). Die in 90.x entfernten ServiceMonitor-Legacy-Keys (`<component>.serviceMonitor.bearerTokenFile`, `kubelet.serviceMonitor.insecureSkipVerify`, `kubeApiServer.tlsConfig.caSecret`) wurden vorab per `grep` gegen `gitops/apps/monitoring.yaml` geprüft — keiner davon bei uns gesetzt, daher nicht betroffen | Teil der Sammel-Update-Runde 2026-09-13 (zusammen mit cert-manager, sealed-secrets, traefik, CDI). Direkter `targetRevision`-Bump ohne CRD-Pre-Apply (da Operator-Version identisch) — bewusste Abweichung vom sonst für diesen Chart üblichen vorsichtigen Stationsverfahren, da die Datenlage (identischer Operator, keine betroffenen Legacy-Keys) das rechtfertigte. Post-Upgrade: alle Pods Running, Grafana `/api/health` 200, ArgoCD Synced/Healthy |
| 2026-09-14 | 90.2.0 → 91.2.1 | Major | Manuell | Abgeschlossen | Diesmal ein echter Operator-Bump v0.93.1→v0.94.0, laut Policy zwingend manuell mit CRD-Pre-Apply. Breaking Changes geprüft, keine betrifft uns: Alertmanager verwirft jetzt Zero-Duration-Felder (`retention`/`clusterGossipInterval`/`clusterPushpullInterval`/`clusterPeerTimeout`) statt sie als CLI-Flag durchzureichen (Startup-Crash-Risiko) — wir setzen nur `retention: 120h` (non-zero), die anderen drei bleiben unset, nicht `0`; Webhook-`matchConditions`-Rendering-Änderung (PR #7275) betrifft nur `prometheusOperator.admissionWebhooks`-Customization, die wir nicht nutzen; RBAC-Verschärfung (keine Wildcard-Verben mehr im Operator-ClusterRole) ist rein additiv-restriktiv. CRD-Schema-Änderungen laut Operator-Release-Notes nur additiv (neue Felder bei ScrapeConfig/Prometheus/Alertmanager-CRD), ServiceMonitor/PodMonitor/PrometheusRule unverändert | Standard-Stationsverfahren: Backup (PrometheusRules/Monitors/Dashboards/Alertmanager-Secret), alle 10 CRDs vorab `server-side --force-conflicts` applyed, dann `targetRevision`-Bump. Kurzer Anzeige-Lag direkt nach dem Sync: `kubectl get deployment` zeigte für ~10-20s noch das alte Operator-Image, `argocd app diff` zeigte kurz veraltete CRD-Annotationen im Cache — beides reine Propagations-/Cache-Verzögerung, kein echtes Problem, mit `argocd app get --hard-refresh` sofort korrekt bestätigt. Alertmanager lief seit diesem Upgrade auf v0.34.0 (neuer Chart-Default), Prometheus unverändert v3.14.0-distroless. Post-Upgrade: alle Pods Running, Prometheus/Alertmanager `RECONCILED=True`/`AVAILABLE=True`, keine Operator-Log-Fehler, 35 PrometheusRules + 13 ServiceMonitors weiterhin vorhanden, Longhorn durchgehend healthy (der historisch riskante Teil bei diesem Chart), Grafana `/api/health` 200 |
| 2026-09-20 | 91.2.1 → 91.4.1 | Minor | Manuell | Abgeschlossen | Operator-Version unverändert (v0.94.0), kein CRD-Update nötig. Subcharts kube-state-metrics 8.4.2→8.5.0 und grafana 13.2.4→13.2.5 gebumpt. kube-state-metrics-Pod-Labels lassen jetzt `helm.sh/chart` weg (verhindert unnötige Pod-Neustarts bei künftigen Chart-Bumps, die das Chart-Label sonst bei jedem Bump ändern würde) — dadurch in diesem Zug ein einmaliger kube-state-metrics-Pod-Neustart (erwartet, bestätigt). Neues optionales Grafana-Operator-Datasource-Feature (`grafana.operator.datasourcesEnabled`, Default `false`) von uns nicht genutzt | Teil der Update-Runde 2026-09-19/20 (zusammen mit traefik, k3s v1.37.0, ArgoCD v3.5.3). Auto-Sync via `selfHeal` lief ohne manuellen Eingriff durch (Push→Sync <1 Min). Post-Upgrade: alle Pods Running, Longhorn healthy, Grafana/Prometheus/Alertmanager unauffällig |
| 2026-09-26 | 91.4.1 → 91.6.0 | Minor | Manuell | Abgeschlossen | Operator v0.94.0→v0.94.1 (Patch), Prometheus v3.14.0→v3.15.0, Alertmanager v0.34.0→v0.34.1. Keine Breaking Changes (Prometheus 3.15 deprecatet nur `--log.level`, von uns nicht gesetzt). CRD-Diff rein additiv (alle 10 CRDs trotzdem vorab server-side applied, Standardprozedur), Operator-ClusterRole-Diff ist reine Umstrukturierung (finalizers-Regel in eigenen Block verschoben, gleiche effektiven Rechte) | Anlass: Nutzer erhielt zuvor Benachrichtigungen über vermeintliche "Prometheus-Crashloops" — Untersuchung ergab, der Prometheus-Pod selbst hatte 0 Restarts und keine CrashLoopBackOff-Alerts; tatsächliche Ursache war ein `io-watchdog`-Bug (fehlendes `O_TRUNC`, siehe `k3s.md`-Stolperfalle), der `k3s-04a` mehrfach grundlos neu startete und dadurch die Longhorn-Replica des Prometheus-Volumes (`pvc-73e5e5c2`) wiederholt in R/W-Timeout schickte → `PrometheusOutOfOrderTimestamps`-Alerts. Bug gefixt und ausgerollt, bevor dieses Chart-Update lief. Post-Upgrade: alle 3 Kern-Images (Operator/Prometheus/Alertmanager-Config-Reloader) korrekt auf neuen Versionen, kurzer bekannter ArgoCD-CRD-Sync-Cache-Lag (~1 Min, siehe Stolperfalle vom 91.2.1→91.2.1-Hop) selbst aufgelöst, keine neuen Prometheus-Log-Fehler, alle 36 PrometheusRules + 14 ServiceMonitors weiterhin vorhanden, Grafana erreichbar |
