...
| unbekannt (vor 2026-09-20) | 3.5.1 → 3.5.2 | Minor (Patch) | vermutlich Script | Abgeschlossen | Nachträglich erkannt: live lief bereits 3.5.2 statt des im Runbook zuletzt dokumentierten 3.5.1 — Hop fand zwischen den Sessions statt und wurde nicht dokumentiert | Nachgetragen am 2026-09-20 im Zuge des 3.5.2→3.5.3-Hops; keine Details zum genauen Ausführungsdatum/-weg rekonstruierbar (kein Git-Commit, da ArgoCD-Version nicht gitops-getrackt ist) |
| 2026-09-20 | 3.5.2 → 3.5.3 | Minor (Patch) | Script (`upgrade-argocd-hop.sh`) | Abgeschlossen | Reiner Bugfix-Release (Health-Checks für KubeVirt-VM/FlinkDeployment/Crossplane/GRPCRoute/GrafanaFolder, Sync-Retry-State-Fix, Repo-Cleanup bei Revision-Wechsel, UI-SSO-Redirect-Loop-Fix), keine Breaking Changes, kein CRD-Schema-Change | Teil der Update-Runde 2026-09-19/20 (zusammen mit traefik, kube-prometheus-stack, k3s v1.37.0 — Reihenfolge: k3s zuerst, da das Master-Upgrade den API-Server kurzzeitig pro Node unerreichbar macht). Lief beim ersten Versuch fehlerfrei durch, NodeAffinity/Resources unverändert erhalten (`statefulset ... patched (no change)`), Controller auf `gmkt-03x`, alle 29 Apps sofort Synced/Healthy |
