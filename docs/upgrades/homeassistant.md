...
| 2026-09-14 | 2026.8.2 → — | Deinstallation | Manuell | Abgeschlossen | HA läuft seit 01.09.2026 stabil als Container auf der diskstation; Cluster-Deployment war seither nur noch auf `replicas: 0` geparkt (ArgoCD-Automatik bereits am 01.09. abgeschaltet). Eingehende Upgrade-Benachrichtigung (2026.8.2 → 2026.9.2) zum Anlass genommen, das Deployment endgültig zu entfernen statt weiter zu pflegen | ArgoCD-App `homeassistant` + `gitops/config/homeassistant/` aus Git entfernt (Prune über `root-infrastructure` app-of-apps), Namespace/Deployment/PVC/Secrets im Cluster gelöscht, Eintrag aus `scripts/upgrade-agent.py` (`IMAGE_SERVICES`) entfernt, offene Bot-PR/Branch für 2026.9.2 geschlossen. **Dieses Runbook wird nicht mehr vom Upgrade-Agent bedient** — künftige HA-Updates laufen ausschließlich auf der diskstation, außerhalb dieses Repos |
