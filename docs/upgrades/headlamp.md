...
## Metadaten
- **Namespace:** `headlamp`
- **Aktuelle Version:** `v0.45.0` (Image `ghcr.io/headlamp-k8s/headlamp:v0.45.0`)
...
| 2026-08-03 | `v0.43.0` → `v0.44.0` | Minor | Manuell | Abgeschlossen | Keine Breaking Changes laut Release Notes — Plugin-API nur additiv erweitert (neuer `entryType: 'subheader'` für `registerSidebarEntry`, neue `registerHomeSidebarEntryFilter`-API), keine CLI-Flag-Entfernungen (`-in-cluster`/`-session-ttl` unberührt); Performance-/Skalierungs-Verbesserungen (Pod-Pagination, Brotli-Precompression) ohne Auswirkung auf unsere Konfiguration | `upgrade-agent` hatte den Sprung bereits als PR-Branch vorbereitet (`chore/upgrade-headlamp-0.44.0`), Branch war aber auf veraltetem Basisstand — nicht gemergt, nur die zwei Image-Tag-Zeilen (Hauptcontainer + `fix-static-plugins`) manuell übernommen. Beide Plugins (Longhorn, Cert-Manager) nach dem Upgrade erfolgreich installiert (Init-Container-Logs geprüft), HTTP 200, keine neuen Fehler im Hauptcontainer-Log |
| 2026-08-30 | `v0.44.0` → `v0.45.0` | Minor | Manuell (Branch von `upgrade-agent` übernommen) | Abgeschlossen | `upgrade-agent` hatte den Branch `chore/upgrade-headlamp-0.45.0` als "manuelle Prüfung nötig" markiert wegen der neuen Security-Funktion "verify external plugin archives". Release-Notes-Detailprüfung ergab: Die Verifikation greift nur für `HEADLAMP_BUILD_MANIFEST`-basierte **Produkt-Builds** (Build-Zeit, White-Label-Distributionen), nicht für Laufzeit-Laden über `-plugins-dir` — betrifft unser Init-Container-Setup also nicht. Keine weiteren Breaking Changes laut Release Notes | Branch gemergt (cherry-pick, da main inzwischen weitergelaufen war) und Remote-Branch gelöscht. Nach Rollout: Plugin-Dateien im Pod vorhanden (`/headlamp/plugins/{headlamp-longhorn,cert-manager}/main.js`), beide über `https://headlamp.reckeweg.io/plugins/.../main.js` mit HTTP 200 und validem JS-Inhalt ausgeliefert (curl-verifiziert), Init-Container-Logs fehlerfrei, Hauptcontainer-Log ohne Plugin-Fehler. Visuelle Browser-Prüfung der Longhorn-/Cert-Manager-Ansicht in dieser Session **nicht möglich** — die Browser-Sandbox blockierte die JS-Assets clientseitig (`ERR_BLOCKED_BY_CLIENT`), obwohl dieselben URLs per `curl` sauber HTTP 200 liefern. Empfehlung: bei Gelegenheit einmal manuell im echten Browser gegenchecken |
